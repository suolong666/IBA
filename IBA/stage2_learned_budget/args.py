...
from IBA.common.data import SeqRecDataset

def parse_global_args(parser):
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument("--base_model", type=str, default="", help="base model or Stage-1 checkpoint path")
    parser.add_argument("--c1_max_k", type=int, default=3, help="maximum refinement depth used by the learned budget module")
    parser.add_argument("--c1_enable_learned_budget", action="store_true", default=False,
                        help="enable learned position-dependent budget allocation")
    parser.add_argument("--c1_learned_budget_total", type=int, default=6,
                        help="total refinement budget B for learned allocation")
    parser.add_argument("--c1_learned_budget_kmax", type=int, default=3,
                        help="maximum candidate refinement depth K_max for learned allocation")
    parser.add_argument("--c1_budget_loss_weight", type=float, default=1.0,
                        help="weight of budget-prediction supervision loss")
    parser.add_argument("--c1_budget_hidden_dim", type=int, default=256,
                        help="hidden width of the budget predictor")
    parser.add_argument("--c1_budget_prior_weight", type=float, default=0.1,
                        help="L1 prior penalty weight toward the information-gain prior schedule")
    parser.add_argument("--c1_budget_prior_schedule", type=str, default="3,2,1,0",
                        help="prior schedule used as an information-gain bias, e.g. '3,2,1,0'")
    parser.add_argument("--c1_budget_monotonic", action="store_true", default=False,
                        help="restrict feasible learned budgets to non-increasing schedules")
    parser.add_argument("--c1_commit_lookahead_weight", type=float, default=0.0,
                        help="weight of lookahead loss from refined semantic states")
    parser.add_argument("--c1_commit_lookahead_max_offset", type=int, default=2,
                        help="max future semantic positions predicted by lookahead")
    parser.add_argument("--c1_tail_recurrence_gamma", type=float, default=0.5,
                        help="mixing weight of previous vertical-tail state in recurrent refinement")
    parser.add_argument("--c1_detach_recurrence", action="store_true", default=False,
                        help="detach recurrent hidden states between refinement steps")
    parser.add_argument("--c1_aux_detach_backbone", action="store_true", default=False,
                        help="stop gradients from auxiliary losses into the backbone")
    parser.add_argument("--max_grad_norm", type=float, default=1.0, help="max grad norm for gradient clipping")
    return parser
...
def parse_dataset_args(parser):
    ...
    parser.add_argument("--index_file", type=str, default="", help="the item indices file")
...
def parse_train_args(parser):
    ...
    parser.add_argument("--finetuned_output_dir", type=str, default="",
...
def parse_test_args(parser):
    ...
    parser.add_argument("--max_new_tokens", type=int, default=10,
                        help="the max number of newly generated tokens during inference")
